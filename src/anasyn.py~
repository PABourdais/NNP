...
class TableSymbole: #Definition de la classe Symbole
	def __init__(self):
		self.table_symbole = []

	def afficherTableSymbole(self):
		for i in self.table_symbole :
			i.afficherScope()

	def ajoutScope(self,tableau_identificateur):
		for j in tableau_identificateur:
			self.table_symbole.append(j)

class Scope: #Definition de la classe Scope
	#Constructeur de la classe Scope
	def __init__(self, nom):
		self.nom = nom 
		self.tableau_identificateur = []

	def ajoutIdentificateur(self,identifierTable):
		for i in identifierTable:
			self.tableau_identificateur.append(i)

	def afficherScope(self):
		print str (self.nom)	
		print str("coucou1")			
		for i in self.tableau_identificateur:
			print str("coucou1")
			i.afficherIdentificateur()
...
class Identificateur: #Definition de la classe Identificateur	
	...
	def afficherIdentificateur(self):
		print str("coucou2")
		print str (self.nom+self.nature)
...
def procedure(lexical_analyser):
	...
	global scope
	lexical_analyser.acceptKeyword("procedure")
	#On a trouve le nom d'une procedure
	ident = lexical_analyser.acceptIdentifier()
	scope = Scope(ident)
	tablesymbole = TableSymbole()
	tablesymbole.ajoutScope(scope)
...
def nnpType(lexical_analyser):
	...
	if lexical_analyser.isKeyword("integer"):
		...
		#On modifie la nature de l'identificateur en integer
		for i in tmpVectorTable:			
			i.setNature("integer")
			identifierTable.append(i) 
		logger.debug("integer type")
	elif lexical_analyser.isKeyword("boolean"):
		lexical_analyser.acceptKeyword("boolean")
		#On modifie la nature de l'identificateur en boolean
		for i in tmpVectorTable:		
			i.setNature("boolean")
			identifierTable.append(i)
		scope.ajoutIdentificateur(identifierTable)
		tablesymbole.ajoutScope(scope.tableau_identificateur)
		#tablesymbole.tableau_identificateur.append(identifierTable)
...
def declaVar(lexical_analyser):
	listeIdent(lexical_analyser)
	#On recupere le nom
	#tmpVectorTable.append(identificateur)
	lexical_analyser.acceptCharacter(":")
	logger.debug("now parsing type...")
	nnpType(lexical_analyser)
	#On recupere le type
	lexical_analyser.acceptCharacter(";")
	#On met a jour la table des identificateur
...
def listeIdent(lexical_analyser):
	...
	#On cree un objet Identificateur
	identificateur = Identificateur(ident)
	tmpVectorTable.append(identificateur)
	...
	program(lexical_analyser)
        ...
        if args.show_ident_table:
                #print "------ IDENTIFIER TABLE ------"
								#for j in tablesymbole.table_symbole :
									#j.afficherScope()
								tablesymbole.afficherTableSymbole()
								#for i in identifierTable :
								#	print str(i.nom +" "+i.nature)
								#ident local a une procedure pp
